...
def insert_into_grid(x, y, z, state, grid):
  if x not in grid:
    grid[x] = {}
...
def get_value_from_grid(x, y, z, grid):
  if x not in grid:
    return "."
...
for y in range(len(input_lines)):
  ...
  for x in range(len(line)):
    insert_into_grid(x, y, 0, line[x], grid)

def print_grid_z(grid, z):
  ...
  for y in range(min_y, max_y+1):
    ...
    for x in range(min_x, max_x+1):
      ...
      line = line + get_value_from_grid(x, y, z, grid)
    print(line)
  ...
  for x in range(min_x-1, max_x+2):
    for y in range(min_y-1, max_y+2):
      for z in range(min_z-1, max_z+2):
        ...
        curr_cube = get_value_from_grid(x, y, z, grid)
        count_neighbors = count_active_neighbors(x, y, z, grid)

        if curr_cube == "#":
          if count_neighbors == 2 or count_neighbors == 3:
            insert_into_grid(x, y, z, "#", next_grid)
          else:
            insert_into_grid(x, y, z, ".", next_grid)
        else:
  ...
  for i in range(-1, 2):
    for j in range(-1, 2):
      for k in range(-1, 2):
        ...
        neighbor = get_value_from_grid(x+i, y+j, z+k, grid)
        if neighbor == "#":
          count += 1
  return count
  ...
  for x in range(min_x, max_x+1):
    for y in range(min_y, max_y+1):
      for z in range(min_z, max_z+1):
        if get_value_from_grid(x, y, z, grid) == "#":
          count += 1
  return count
